...
namespace MPPPenAutoTuning
{
    public sealed class InputDevice
    {
        ...
        public class HIDDeviceEventArgs : EventArgs
        {
            ...
            /// The device handle
            /// </summary>
            public IntPtr m_hDevice = IntPtr.Zero;

            public string m_sDevicePath = "";

            public HIDDeviceEventArgs(byte[] Buffer, IntPtr hDevice, string sDevicePath)
            {
                m_Buffer = Buffer;
                m_hDevice = hDevice;
        ...
        public InputDevice(IntPtr hwnd, frmMain cfrmMain)
        {
            m_hWnd = hwnd;
            m_cfrmMain = cfrmMain;
        }
        ...
        public bool RegisterHIDDevice(int nVID, int nPID, string sDeviceID = "", ushort nCurUsagePage = 0x0d)
        {
            //Get the Device List
            uint nInputDeviceCount = 0;
            int nSize = Marshal.SizeOf(typeof(RAWINPUTDEVICELIST));
            bool bFindDeviceFlag = false;
            IntPtr pRawInputDeviceList = IntPtr.Zero;
            int nResult = -1;

            if (GetRawInputDeviceList(IntPtr.Zero, ref nInputDeviceCount, (uint)nSize) != 0)
            {
                //Program.DebugLog.WriteLog("Failed to Get raw input device Count.");
                OutputMessage("-Fail to Get raw input device Count");
                return false;
            }
            ...
            if (nResult == -1)
            {
                //Debug.WriteLine("Failed to Get raw input device(s).");
                //Program.DebugLog.WriteLog("Failed to Get raw input device(s).");
                OutputMessage("-Fail to Get raw input device(s)");
                return false;
            }

            OutputMessage(string.Format("-RawInputDeviceCount(0x{0}) : {1}", nCurUsagePage.ToString("X4"), nInputDeviceCount.ToString()));
            ...
            for (int i = 0; i < nInputDeviceCount; i++)
            {
                ...
                if (sDeviceID != "")
                {
                    string sDevName = GetDeviceName(InputDevice.hDevice);

                    if (sDevName == "" || sDevName.Contains(sDeviceID) == false || sDevName.Contains("Col02") == false)
                    {
            ...
            if (bFindDeviceFlag == false)
            {
                //Debug.WriteLine("Failed to Get raw input device(s).");
                //Program.DebugLog.WriteLog("Can't find out the indicated device(s).");
                OutputMessage("-Can't find out the indicated device(s)");
                return false;
            }
            ...
            if (RegisterRawInputDevices(RawInputDevice, 1, (uint)Marshal.SizeOf(typeof(RAWINPUTDEVICE))) == false)
            {
                ...
                //Program.DebugLog.WriteLog("RegisterRawInputDevices Failed");
                OutputMessage("-RegisterRawInputDevices Fail");
                return false;
            }
        ...
        /// <returns></returns>
        public IntPtr GetDeviceHandle(int nVID, int nPID, string sDeviceID = "")
        {
            IntPtr hFindDevice = IntPtr.Zero;
            //Get the Device List
            uint nInputDeviceCount = 0;
            int nSize = Marshal.SizeOf(typeof(RAWINPUTDEVICELIST));
            IntPtr pRawInputDeviceList = IntPtr.Zero;
            ...
            for (int i = 0; i < nInputDeviceCount; i++)
            {
                ...
                if (devInfo.dwType == RIM_TYPEHID)
                {
                    ...
                    hFindDevice = InputDevice.hDevice;
                    Marshal.FreeHGlobal(pDevInfo);
                    break;
                }
        ...
        {
            int nResult = 0;
            uint nRIDDevNameSize = 0;
            string sDeviceName = "";
            IntPtr pNameData = IntPtr.Zero;

            nResult = (int)GetRawInputDeviceInfo(hDevice, RIDI_DEVICENAME, IntPtr.Zero, ref nRIDDevNameSize);
            pNameData = Marshal.AllocHGlobal((int)nRIDDevNameSize);

            nResult = (int)GetRawInputDeviceInfo(hDevice, RIDI_DEVICENAME, pNameData, ref nRIDDevNameSize);

            if (nResult != -1)
                sDeviceName = Marshal.PtrToStringAnsi(pNameData);

            Marshal.FreeHGlobal(pNameData);
                                     ...
                                     (uint)Marshal.SizeOf(typeof(RAWINPUTHEADER))) == dwSize)
                {
                    ...
                    else if (raw.header.dwType == RIM_TYPEHID)
                    {
                        for (int index = 0; index < raw.Data.hid.dwCount; ++index)
                        {
                            ...
                            string sDevName = GetDeviceName(raw.header.hDevice);
                            HIDHandler(this, new HIDDeviceEventArgs(pBuffer, raw.header.hDevice, sDevName));
                        }
                    }
                }
            }

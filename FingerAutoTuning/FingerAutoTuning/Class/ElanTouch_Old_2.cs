...
namespace Elan
{
    public class ElanTouch
    {
        ...
        {
            if ((Mode & ElanTouch.TraceMode.Self) != 0 || (Mode & ElanTouch.TraceMode.ComboSelf) != 0)
                return true;
            else
                return false;
        }
        ...
        public static TraceMode GetTraceMode(int nICType, bool bSelf = true)
        {
            ...
            if (bSelf == true)
            {
                switch (nICType)
                {
                    case ElanDef.Gen53_ICType:
                        Mode |= TraceMode.Self;
                        break;
                    case ElanDef.Gen63_ICType:
                    case ElanDef.Gen5M_ICType:
                    case ElanDef.Gen39P_ICType:
        ...
        */
        ...
        [DllImport("LibTouch.dll", SetLastError = true)]
        public static extern int ResetBridge(int nVID, int nPID, InterfaceType eInterfaceType);
        ...
        /// </summary>
        /// <returns></returns>
        public static int GetFWIPOption(int nDevIdx = 0)
        {
            int nRetryCount = RETRY_COUNT;
            int nValue = 0;
            while (nRetryCount > 0)
            {
                int nRet = GetFWIPOption(ref nValue, 1000, nDevIdx);
                if (nRet == TP_SUCCESS)
                    break;
